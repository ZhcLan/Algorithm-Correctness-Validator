...
public class RPC {
     ...
     */
    public static Object[][] rpc(ValidatorConfig config, Argument[] arguments, StringBuilder error)
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            args[i] = rsc(arguments, types[i], i, 0, error, null);
            error.append("}");
            if (i != types.length - 1) {
                error.append(",");
            }
     ...
     */
    /* unchecked */
    public static Object[] rsc(Argument[] arguments, Type root,
                               int index, int used, StringBuilder error, Range range)
            throws ClassNotFoundException {
        Object[] ret = new Object[2];
        try {
            // Map or Collection
            // create root
            ret[0] = ReflectiveInvoker.getInstance(((ParameterizedType) root).getRawType().getTypeName(), null, null);
            ret[1] = ReflectiveInvoker.getInstance(((ParameterizedType) root).getRawType().getTypeName(), null, null);
            ...
            if (root instanceof GenericArrayType) {
                ...
                for (int i = 0; i < size; i++) {
                    Object[] subArray = rsc(arguments, ((GenericArrayType) root).getGenericComponentType(), index, used + 1, error, null);
                    if (i != size - 1) {
                        error.append(",");
                    }
                ...
                try {
                    ...
                    error.append("[");
                    for (int i = 0; i < size; i++) {
                        Object[] subArray = rsc(arguments, ((Class<?>) root).getComponentType(), index, used + 1, error, null);
                        if (i != size - 1) {
                            error.append(",");
                        }
                        Array.set(array1, i, subArray[0]);
                ...
                Range random = arguments[index].getValues()[used];
                if (root.getTypeName().contains("String")) {
                    DataSet set = arguments[index].getVolume()[used].getDataSet();
                    int len = (int) Range.getRandomMinToMax(arguments[index].getVolume()[used]);
                    if (len == 0) {
                    ...
                    if (set == null) {
                        ...
                        ret[0] = new StringPlus(set, random, len);
                    }
                    ...
                    DataSet set = null;
                    if (used - 1 >= 0) {
                        set = arguments[index].getVolume()[used - 1].getDataSet();
                    }
                    if (range != null) {
                        ret[0] = ReflectiveInvoker.getInstance(root.getTypeName(), set, range);
                        error.append(ret[0]);
                    } else {
                        ret[0] = ReflectiveInvoker.getInstance(root.getTypeName(), set, random);
                        error.append(ret[0]);
                    }
        ...
        if (types.length == 2) {// Interface Map
            ...
            while (count < size) {
                ...
                error.append("(");
                do {
                    rightKey = rsc(arguments, keyType, index, used + 1, error, null);
                } while (map1.containsKey(rightKey[0]));
                error.append(",");

                Object[] rightVal = rsc(arguments, valType, index, used + 2, error, null);
            ...
            if (fateful != null) {
                ...
                for (int i = 0; i < times.length; i++) {
                    for (int j = 0; j < times[i]; j++) {
                        Range fate = fateful.getValue()[i];
                        // Returns two leaf nodes
                        Object[] objects = rsc(arguments, colType, index, used + 1, error, fate);
                        error.append(",");
                        list1.add(objects[0]);
                        list2.add(objects[1]);
                        count++;
                ...
                while (count < size) {
                    ...
                    Object[] objects = rsc(arguments, colType, index, used + 1, error, null);
                    if (count != size - 1) {
                        error.append(",");
                    }
                    list1.add(objects[0]);
