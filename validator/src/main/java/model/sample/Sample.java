 ...
 */
public class Sample {
    public Object[] validatorSample;// Sample logarithmic method
    public Object[] cloneSample;    // The logarithmic sample clone uuid & value is the same
    public String errorSample;      // Error Sample is used to output error information
    /**
     * constructor
     *
     * @param config logarithmic config
     * @param volume amount of data
     * @param values numeric rang
     */
    public Sample(ValidatorConfig config, Range[][] volume, Range[][] values) {
        try {
            StringBuilder error = new StringBuilder();      // Output-type parameters
            Object[][] parameter = RPC.rpc(config, volume, values, error);
     ...
     *
     * @param validatorResultRet
     * @param compareResultSet
     * @param config
     * @return
     */
    public boolean compareResultSet(List<Object> validatorResultRet, List<Object> compareResultSet, ValidatorConfig config) {
        if (validatorResultRet == null && compareResultSet != null) {
            System.out.print("\n\n" + GlobalUtility.RED + "Verification failed, please adjust the sample size debug!" + GlobalUtility.RESET);
            System.out.println("\nsample:" + errorSample);
            System.out.println(config.getValidatorMethod() + ":" + null);
            System.out.println(config.getCompareMethod() + ":" + compareResultSet);
            System.out.println(GlobalUtility.RED + "validator result set is empty and compare result set is not empty!" + GlobalUtility.RESET);
            return false;
        }
        ...
        if (validatorResultRet != null && compareResultSet == null) {
            ...
            System.out.println(GlobalUtility.RED + "compare result set is empty and validator result set is not empty!" + GlobalUtility.RESET);
        }
        ...
        if (validatorResultRet.size() != compareResultSet.size()) {
            ...
            System.out.println(config.getValidatorMethod() + ":" + validatorResultRet);
            System.out.println(config.getCompareMethod() + ":" + compareResultSet);
            System.out.println(GlobalUtility.RED + "The length of validator result set and compare result set is different!" + GlobalUtility.RESET);
            return false;
        }
        ...
        if (config.isComparison()) {
            for (int i = 0; i < validatorResultRet.size(); i++) {
                ...
                if (result == 0) {
                    ...
                    System.out.println(GlobalUtility.RED + "The addresses and values of the validator result set and compare result set elements are not equal!" + GlobalUtility.RESET);
                    return false;
                } else if (result == -2) {
                    // v1 = v2 && i1 != i2
                    System.out.print("\n\n" + GlobalUtility.RED + "Verification failed, please adjust the sample size debug!" + GlobalUtility.RESET);
                    System.out.println("\nsample:" + errorSample);
                    System.out.println(Out.outValueAndUUID(validatorResultRet, compareResultSet));
                    System.out.println(GlobalUtility.RED + "The addresses of the validator result set and compare result set elements are not equal!" + GlobalUtility.RESET);
                    return false;
                } else if (result == -1) {
                    // v1 != v2 && i1 = i2
                    System.out.print("\n\n" + GlobalUtility.RED + "Verification failed, please adjust the sample size debug!" + GlobalUtility.RESET);
                    ...
                    System.out.println(config.getValidatorMethod() + ":" + validatorResultRet);
                    System.out.println(config.getCompareMethod() + ":" + compareResultSet);
                    System.out.println(GlobalUtility.RED + "The values of the validator result set and compare validator elements are not equal!" + GlobalUtility.RESET);
                    return false;
                }
            }
        } else {
            ...
            for (int i = 0; i < validatorResultRet.size(); i++) {
                ...
                if (result == 0 || result == -1) {
                    ...
                    System.out.println(config.getValidatorMethod() + ":" + validatorResultRet);
                    System.out.println(config.getCompareMethod() + ":" + compareResultSet);
                    System.out.println(GlobalUtility.RED + "The values of the list1 and list2 elements are not equal," + GlobalUtility.RESET);
                    return false;
                }
            }
        }
